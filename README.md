...
- Scans in three sequential priority chunks — **last 24h**, then **24h–7d**, then **7d–30d** —
  so the 24h number finishes completely (and gets the full rate-limit budget) before any
  requests are spent on older data. Each chunk uses a few concurrent sub-streams internally
  for speed, staggered slightly and sharing one global rate-limit cooldown, so a 429 in one
  stream backs off the whole chunk rather than just itself.
- Pages backward through `GET https://quaiscan.io/api/v2/transactions?filter=validated`
  using Quaiscan's keyset pagination (`next_page_params`), seeding later chunks at an
  estimated starting block (calibrated from two block-list reads) instead of walking
  through everything newer first.
